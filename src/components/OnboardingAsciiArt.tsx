...
export function OnboardingAsciiArt() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          ...
          for (let sr = 0; sr < sprite.length; sr += 1) {
            const line = sprite[sr]!;
            for (let sc = 0; sc < line.length; sc += 1) {
              const ch = line[sc] ?? " ";
              if (ch === " ") continue;
    ...
    function stampClip(clip: Clip, alpha: number) {
      ...
      for (let sr = 0; sr < clip.sprite.length; sr += 1) {
    ...
    function step(time: number) {
      ...
      for (let i = 0; i < trail.length; i += 1) trail[i] *= 0.92;
      clipMask.fill(0);
      ...
      for (let c = 0; c < cols; c += 1) colWave[c] = Math.sin(c * 0.08 + tick * 0.06);
      for (let r = 0; r < rows; r += 1) rowWave[r] = Math.cos(r * 0.1 - tick * 0.05);
  ...
  return (
    <pre
      ref={preRef}
      className="h-full w-full overflow-hidden p-0 m-0 select-none leading-none text-white/35"
