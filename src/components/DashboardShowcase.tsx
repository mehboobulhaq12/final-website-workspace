...
/* ── Mobile Stat Card ── */
const MobileStatCard = ({ label, value, change, up, icon: Icon, color }: {
  label: string; value: string; change: string; up: boolean; icon: React.ElementType; color: string;
}) => (
  <motion.div
    className="bg-[#111] rounded-2xl border border-white/5 p-4 flex flex-col gap-3"
...
/* ── Mobile Activity Row ── */
const MobileActivityRow = ({ name, initial, color, msg, sentiment, converted }: {
  name: string; initial: string; color: string; msg: string; sentiment: string; converted: string;
}) => (
  <div className="flex items-start gap-3 py-3 border-b border-white/5 last:border-0">
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className={`text-[10px] px-2 py-0.5 rounded-full shrink-0 ${sentiment === "Positive" ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}>
          {sentiment}
        </span>
      </div>
      <p className="text-xs text-white/50 mt-0.5 truncate">"{msg}"</p>
      {converted === "CONVERTED" && (
        <span className="inline-block mt-1 text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-bold">
          CONVERTED
        </span>
      )}
    </div>
...
const MobileDashboard = () => (
  <div className="flex flex-col gap-3">
    {/* Top bar mockup */}
    <div className="bg-[#0d0d0d] rounded-2xl border border-white/5 p-3 flex items-center justify-between">
      <div className="flex items-center gap-2">
        <img src={logoImg} alt="Effect3" className="h-5" />
    ...
    <div className="grid grid-cols-2 gap-3">
      <MobileStatCard label="Leads Uploaded" value="3,598" change="12.5%" up icon={Upload} color="bg-orange-500/20" />
      <MobileStatCard label="Leads Contacted" value="2,096" change="8.2%" up icon={Mail} color="bg-blue-500/20" />
      <MobileStatCard label="Leads Rejected" value="289" change="1.2%" up={false} icon={TrendingDown} color="bg-red-500/20" />
      <MobileStatCard label="Active Conversations" value="69" change="5.2%" up icon={MessageSquare} color="bg-emerald-500/20" />
    </div>
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-white">Outreach Performance</span>
      </div>
    ...
    >
      <span className="text-sm font-semibold text-white">Channel Analytics</span>
      <div className="flex items-center gap-4 mt-3">
        ...
        <div className="flex flex-col gap-2 text-xs">
          ...
          <div className="mt-1 text-white/30">
            <span className="text-orange-400 font-bold text-base">8.5k</span> Leads
          </div>
        </div>
      </div>
    </motion.div>
    ...
    >
      ...
      <MobileActivityRow initial="E" color="bg-purple-500" name="Emma Thompson" msg="Wow, that's really helpful. Thanks!" sentiment="Positive" converted="No" />
      <MobileActivityRow initial="L" color="bg-blue-500" name="Liam Wilson" msg="Can you send me the link?" sentiment="Positive" converted="CONVERTED" />
      <MobileActivityRow initial="S" color="bg-emerald-500" name="Sofia Martinez" msg="I'd like to schedule a demo" sentiment="Positive" converted="No" />
    </motion.div>
  </div>
);
    ...
    <section className="w-full py-16 sm:py-24 bg-black relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-4" ref={ref}>
        ...
        >
          ...
          <p className="text-white/40 mt-2 text-sm max-w-xl mx-auto px-2">
